...
class BlinkDetector:
    ...
    def _merge_blinks(self, blink_onsets, blink_offsets,min_dur,additional_params = []):
        """
        ...
        for i, onset in enumerate(blink_onsets):
            ...
            if i < len(blink_onsets) - 1:
                if ((blink_onsets[i+1] - blink_offsets[i])) < self.settings.min_separation:
    ...
    def blink_detector_eo(self, t, eye_openness_signal_left,eye_openness_signal_right):
        ...
        """
        ms_to_sample = self.settings.Fs / 1000
        sample_to_ms = 1000 / self.settings.Fs

        # Compute eye openness signal for min amplitude not filtered, just averaged
        fully_open = np.nanmedian(preprocessing.compute_average_array(eye_openness_signal_right,eye_openness_signal_left),axis = 0)
        ...
        width_of_blink =self.settings.width_of_blink * ms_to_sample
        filter_length = preprocessing.nearest_odd_integer(self.settings.filter_length * ms_to_sample)
        if filter_length < 3:
            filter_length = 3

        # Interpolate gaps
        eye_openness_signal_right = preprocessing.interpolate_nans(t, eye_openness_signal_right,
                                                            gap_dur=int(self.settings.gap_dur))
        eye_openness_signal_left = preprocessing.interpolate_nans(t, eye_openness_signal_left,
                                                            gap_dur=int(self.settings.gap_dur))
        # Filter eyelid signal and compute average
        ...
        # compute velocity unfiltered data but interpolated nans then average
        eye_openness_signal_vel_left = savgol_filter(eye_openness_signal_left, filter_length, 2,
                                            deriv=1,  mode='nearest') * self.settings.Fs
        eye_openness_signal_vel_right = savgol_filter(eye_openness_signal_right, filter_length, 2,
                                            deriv=1,  mode='nearest') * self.settings.Fs
        eye_openness_signal_vel = preprocessing.compute_average_array(eye_openness_signal_vel_left,eye_openness_signal_vel_right)
        ...
        if len(blink_properties) == 0:
            ...
            bp =  self._merge_blinks(blink_onsets, blink_offsets, width_of_blink,
                            blink_temp[:, 3:])

        # Convert to dataframe
        blink_df = pd.DataFrame(bp,
        ...
        blink_df.iloc[blink_df.openness_at_peak < 0] = 0
        return blink_df
'''
